...
data = corpus
with header:
    st.title('Welcome to GreatLearning CHATBOT!!!')
    st.text(' Please enter your query, The Chat bot will respond')
...
try:
    ...
    # model.save("model_app.tflearn")
    model.load("model_app.tflearn")
except:
    model.fit(train_X,train_y,n_epoch = 1000,batch_size=5,show_metric=True)
    model.save("model_app.tflearn")
    ...
    while not done:
        ...
        if ints[0]['intent'] == 'Exit':
            ...
            if message.lower()=='' or message.lower() == '*':
                responses =  'Please re-phrase your query'
            elif float(ints[0]['probability']) > 0.35:
                responses = response(message)
            else:
                responses= 'Sorry I am still learning! Please Rephrase your query'
        return responses
